...
YSM 通过 Mixin 在 `EntityRenderDispatcher.render()` 级别拦截玩家渲染，绕过原版 `PlayerRenderer`。GoetyRevelation 的 `PlayerHaloLayer`（Ascension Halo）和 RevelationFix 的 `OdamaneHaloLayer`（halo_of_the_end，通过 CuriosLayer 渲染）依赖原版渲染管线，YS  M 会跳过这些图层，导致光环不显示或显示在错误位置。
...
进游戏 → Mods → YSM GoetyRevelation 兼容 → Config → 调整 `headYOffset`（光环头部偏移，单位像素，默认 40）
...
RenderLivingEvent.Post 触发
    ...
    │
    └── 只渲染光环相关层:
        ├── PlayerHaloLayer（GoetyRevelation Ascension Halo）
        └── CuriosLayer → OdamaneHaloLayer（RevelationFix halo_of_the_end）
            │
            ├── setupAnim() 动画化 PlayerRenderer 模型（修复头部旋转跟踪）
            ├── 头部垂直平移（可配置偏移，补偿 ModelPart 层级问题）
